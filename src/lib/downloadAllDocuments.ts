...
export const downloadAllDocuments = async (
  ...
  try {
    // 1. House Rules PDF
    const houseRulesResult = generatePDF({
    ...
    if (!houseRulesResult) errors.push('House Rules PDF');
  } catch {
    errors.push('House Rules PDF');
  }
  ...
  try {
    // 3. Checklist PDF
    const checklistResult = generateChecklistPDF({
    ...
    if (!checklistResult) errors.push('Checklist PDF');
  } catch {
    errors.push('Checklist PDF');
  }
  ...
  try {
    ...
    errors.push('Cancellation Policy PDF');
  }
